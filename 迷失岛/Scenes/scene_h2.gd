extends "res://Scenes/game_scene.gd"

@onready var dialog := $Granny/Dialog
...
func _on_granny_interact():
	...
	if item :
		if item == preload("res://Res/mail.tres"):
			Game.flags.add(flag)
			Game.inventory.del(item)
		else:
			return
			
	if Game.flags.has(flag):
		dialog.show_dialog(["没想到老头子的船票寄过来了，谢谢你。"])
	else :	
		dialog.show_dialog(["我年纪大了，很多事情想不起来了。",
